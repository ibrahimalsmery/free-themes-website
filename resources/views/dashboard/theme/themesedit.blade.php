...
@section('content')
    <div class="card card-body">
        ...
        <form id="deleteForm" class="mb-2" action="{{ route('dashboard.themes.destroy', ['theme' => $theme->id]) }}"
            method="post">
            @method('DELETE')
            @csrf
            <button class="btn btn-danger btn-lg"><i class="fa-solid fa-circle-xmark"></i> DELETE</button>
        </form>
        <script>
        ...
        <form action="{{ route('dashboard.themes.update', ['theme' => $theme->id]) }}" method="post"
            enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="form-group">
                <input type="text" name="title" value="{{ old('title', $theme->title) }}"
            ...
            <fieldset class="form-group row">
                ...
                <div class="col-sm-10">
                    <div class="form-check">
                        <input class="form-check-input" {{ old('status', $theme->status) == 1 ? 'checked' : '' }}
                    ...
                    <div class="form-check">
                        <input class="form-check-input" {{ old('status', $theme->status) == 0 ? 'checked' : '' }}
            ...
            <div class="form-group">
                <input type="text" name="preview_link" value="{{ old('preview_link', $theme->preview_link) }}"
